...
def test_single(device_id, filepath="testing_file/file.mp3", blocksize=1024):
...
def test_multiple(device_ids, filepath="file.mp3"):
    def _runner():
        threads = []
        for i in device_ids:
            t = threading.Thread(target=test_single, kwargs={"device_id": i, "filepath": filepath})
            threads.append(t)
            t.start()
